<?php
$MESS ['IBLOCK_IBLOCK_URL'] = "URL, ведущий на страницу с содержимым раздела";
$MESS ['IBLOCK_DETAIL_URL'] = "URL, ведущий на страницу с содержимым элемента раздела";
$MESS ['T_IBLOCK_DESC_ASC'] = "По возрастанию";
$MESS ['T_IBLOCK_DESC_DESC'] = "По убыванию";
$MESS ['T_IBLOCK_DESC_FID'] = "ID";
$MESS ['T_IBLOCK_DESC_FNAME'] = "Название";
$MESS ['T_IBLOCK_DESC_FACT'] = "Дата начала активности";
$MESS ['T_IBLOCK_DESC_FSORT'] = "Сортировка";
$MESS ['T_IBLOCK_DESC_FTSAMP'] = "Дата последнего изменения";
$MESS ['T_IBLOCK_DESC_IBTYPE'] = "Тип информационных блоков";
$MESS ['T_IBLOCK_DESC_IBSORT'] = "Поле для сортировки информационных блоков";
$MESS ['T_IBLOCK_DESC_SORT'] = "Сортировка";
$MESS ['T_IBLOCK_DESC_ID'] = "Идентификатор";
$MESS ['T_IBLOCK_DESC_IBSORTBY'] = "Направление для сортировки информационных блоков";
$MESS ['T_IBLOCK_DESC_IBCNT'] = "Количество записей в каждом блоке";
$MESS ['T_IBLOCK_DESC_IBORD'] = "Поле по которому сортировать записи";
$MESS ['T_IBLOCK_DESC_IBBY'] = "Направление для первой сортировки записей";
$MESS ['T_IBLOCK_DESC_LIST_ID'] = "Код информационного блока";
$MESS ['IBLOCK_FIELD'] = "Поля";
$MESS ['IBLOCK_PROPERTY'] = "Свойства";
$MESS ['T_IBLOCK_DESC_ACTIVE_DATE_FORMAT'] = "Формат показа даты";
$MESS ['IBLOCK_FILTER_NAME_IN'] = "Имя массива со значениями фильтра для фильтрации элементов";
$MESS ['CP_BNI_CACHE_GROUPS'] = "Учитывать права доступа";
